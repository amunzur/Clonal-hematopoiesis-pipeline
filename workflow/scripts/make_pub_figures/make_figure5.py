...
def subset_to_ot_samples(chip):
    """
    Subsets the df and OT mutation dfs to samples taken from baseline, and the subsequent OT timepoint sample. 
    """
    path_ot_samples = "/groups/wyattgrp/users/amunzur/pipeline/resources/sample_lists/bladder_OT_samples.csv"
    ot_samples = pd.read_csv(path_ot_samples)
    ot_samples["Date_collected"] = pd.to_datetime(ot_samples["Date_collected"])
    ot_samples = ot_samples[["Patient_id", "Date_collected"]]
    
    df_baseline = chip[(chip["Dependent"] == False) & (chip["Timepoint"] == "Baseline")][["Patient_id", "Timepoint", "Gene"]].reset_index(drop = True)
    df_OT = chip[(chip["Dependent"] == False) & (chip["Timepoint"] == "During treatment")][["Patient_id", "Timepoint", "Date_collected", "Gene"]].reset_index(drop = True)  
    
    # Subset to relevant patients and samples
    df_baseline = df_baseline[df_baseline["Patient_id"].isin(ot_samples["Patient_id"].unique())]
    df_OT["Date_collected"] = pd.to_datetime(df_OT["Date_collected"], format='%Y%b%d')
    df_OT= df_OT.merge(ot_samples)[["Patient_id", "Timepoint", "Gene"]]
    df_baseline = df_baseline[df_baseline["Patient_id"].isin(ot_samples["Patient_id"])].reset_index(drop = True)    
    
    # Distribution of genes
    df_baseline.loc[df_baseline["Gene"].isin(["STAG2", "ARID1A", "CBL", "PBRM1", "MYD88", "JAK2", "EZH2", "TERT", "GNAS", "RAD21", "ERBB2", "RUNX1", "KMT2D", "SETD2"]), "Gene"] = "Other"
    df_OT.loc[df_OT["Gene"].isin(["STAG2", "CBL", "PBRM1", "MYD88", "JAK2", "EZH2", "TERT", "GNAS", "RAD21", "ERBB2", "RUNX1", "KMT2D", "SETD2"]), "Gene"] = "Other"
    
    return(df_baseline, df_OT)
...
def plot_the_bar(chip, ax, ax_legend):
    ...
    df_baseline, df_OT = subset_to_ot_samples(chip)
    path_ot_samples = "/groups/wyattgrp/users/amunzur/pipeline/resources/sample_lists/bladder_OT_samples.csv"
    ot_samples = pd.read_csv(path_ot_samples)
    ot_samples["Date_collected"] = pd.to_datetime(ot_samples["Date_collected"])
    ot_samples = ot_samples[["Patient_id", "Date_collected"]]
...
# We need to generate a list of mUC patients to include in this analysis, including an annotation of when their OT blood sample was collected.
# Only interested in the 2nd OT sample only for this analysis.
...
pts_with_ot = sample_info[sample_info["Timepoint"] == "During treatment"]["Patient_id"].unique()
sample_info = sample_info[sample_info["Patient_id"].isin(pts_with_ot)].reset_index(drop = True)
sample_info.to_csv("/groups/wyattgrp/users/amunzur/pipeline/resources/sample_lists/bladder_OT_samples.csv", index = False)

# Group by Patient_id
def closest_to_baseline(group):
    # Separate baseline and "During treatment" samples
    baseline = group[group["Timepoint"] == "Baseline"]
    during_treatment = group[group["Timepoint"] == "During treatment"]
    
    # If more than one "During treatment" sample, pick the closest to baseline
    if len(during_treatment) > 1:
        baseline_date = baseline["Date_collected"].values[0]
        during_treatment["date_diff"] = abs(during_treatment["Date_collected"] - baseline_date)
        return pd.concat([baseline, during_treatment.loc[during_treatment["date_diff"].idxmin()]])
    else:
        return pd.concat([baseline, during_treatment])

# Filter patients that have more than one timepoint and apply the function
sample_info_ot = sample_info.groupby('Patient_id').filter(lambda x: (x['Timepoint'] == "During treatment").sum() > 0)
sample_info_ot = sample_info_ot.groupby('Patient_id').apply(closest_to_baseline).reset_index(drop=True)
...
# RELEVANT STATS FOR THE PAPER
# Any difference in gene prevalence between baseline and OT, in patients that provided two samples?
for i, gene in enumerate(genes_list):
    mutations, CIs = get_baseline_and_ot_vafs(chip, gene, pts_with_ot, return_difference = False) # get the vafs all mutations in a given gene from baseline to timepoint2

# Median time difference between baseline and the subsequent blood collection. 
path_ot_samples = "/groups/wyattgrp/users/amunzur/pipeline/resources/sample_lists/bladder_OT_samples.csv"
ot_samples = pd.read_csv(path_ot_samples)
ot_samples["Date_collected"] = pd.to_datetime(ot_samples["Date_collected"])

mylist = []
for i, group in ot_samples.groupby("Patient_id"):
    baseline_date = group[group["Timepoint"] == "Baseline"].reset_index(drop = True)["Date_collected"].iloc[0]
    ot_date = group[group["Timepoint"] == "During treatment"].reset_index(drop = True)["Date_collected"].iloc[0]
    
    time_diff = abs((baseline_date - ot_date).days/7)
    mylist.append(time_diff)

median = np.median(mylist)
Q1 = np.percentile(mylist, 25)  # 25th percentile
Q3 = np.percentile(mylist, 75)  # 75th percentile

# Diffrerence in CH detection rate in chosen genes. 
gene_colors = {
    "Other": "Silver",
...
df_baseline, df_OT = subset_to_ot_samples(chip)
df_baseline = df_baseline[["Patient_id", "Gene"]].drop_duplicates()
df_baseline = df_baseline["Gene"].value_counts().reset_index().rename(columns = {"index": "Gene", "Gene": "Count"})

df_OT = df_OT[["Patient_id", "Gene"]].drop_duplicates()
df_OT = df_OT["Gene"].value_counts().reset_index().rename(columns = {"index": "Gene", "Gene": "Count"})

# Calculate the denoms, the number of patients considered
path_ot_samples = "/groups/wyattgrp/users/amunzur/pipeline/resources/sample_lists/bladder_OT_samples.csv"
ot_samples = pd.read_csv(path_ot_samples)
n_pts = ot_samples["Patient_id"].unique().shape[0]

df_baseline["perc"] = df_baseline["Count"]/n_pts*100
df_OT["perc"] = df_OT["Count"]/n_pts*100

# Combine the two DataFrames on 'Gene'
df_combined = pd.merge(df_OT[['Gene', 'Count', 'perc']], df_baseline[['Gene', 'Count', 'perc']], on='Gene', how='outer', suffixes=('_OT', '_baseline')).fillna(0)

# Total number of samples in OT and baseline
n_OT = df_OT['Count'].sum()
n_baseline = df_baseline['Count'].sum()

# Initialize a list to store results
results = []

# Perform Z-test for each gene
for index, row in df_combined.iterrows():
    gene = row['Gene']
    count_OT = row['Count_OT']
    count_baseline = row['Count_baseline']
    
    # Define the counts and the total number of observations in both groups
    counts = [count_OT, count_baseline]
    nobs = [n_OT, n_baseline]
    
    # Perform the Z-test for proportions
    stat, pval = proportions_ztest(counts, nobs)
    
    # Store the result
    results.append({'Gene': gene, 'Z-statistic': stat, 'P-value': pval})
...
print("\nSignificant differences found in the following genes:")
