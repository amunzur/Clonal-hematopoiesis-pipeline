...
# clin_df = clin_df[(clin_df["First sample?"] == True) & (clin_df["Event"] == "1L")]
...
chip_status = prepare_chip_status(base_bladder_chip, "Bladder", PATH_sample_information, annotate_gene = "TP53")
...
ctDNA_bladder_KM_ax.text(0.86, 0.60, f"p={p}", transform=ctDNA_bladder_KM_ax.transAxes, fontsize=8, ha='center', va='center')
...
chip_status = prepare_chip_status(base_kidney_chip, "Kidney", PATH_sample_information, annotate_gene = "TP53")
...
gs.tight_layout(fig)
fig.savefig(os.path.join(figure_dir, "TP53_CH.png"))
fig.savefig(os.path.join(figure_dir, "TP53_CH.pdf"))

# Some more info on the bladder TP53 CH+ patients.
clin_df = pd.read_csv(PATH_clinical_bladder)
clin_df = clin_df[clin_df["First sample?"] == True]
chip_status = prepare_chip_status(base_bladder_chip, "Bladder", PATH_sample_information, annotate_gene = "TP53")
pos_pts = chip_status[chip_status["CHIP positive"]]["Patient_id"].tolist()
neg_pts = chip_status[~chip_status["CHIP positive"]]["Patient_id"].tolist()

pos_ctdna = base_bladder_somatic[base_bladder_somatic["Patient_id"].isin(pos_pts)].groupby("Patient_id")["VAF_t"].transform("median").unique()
neg_ctdna = base_bladder_somatic[~base_bladder_somatic["Patient_id"].isin(pos_pts)].groupby("Patient_id")["VAF_t"].transform("median").unique()

np.median(pos_ctdna)
np.median(neg_ctdna)

data = pd.DataFrame({
    "VAF_t": list(pos_ctdna) + list(neg_ctdna),
    "Group": ["TP53 CH Positive"] * len(pos_ctdna) + ["TP53 CH Negative"] * len(neg_ctdna)
})

plt.figure(figsize=(8, 6))
sns.boxplot(x="Group", y="VAF_t", data=data, palette="Set2", showfliers=False)
sns.swarmplot(x="Group", y="VAF_t", data=data, color=".25")
plt.savefig('/groups/wyattgrp/users/amunzur/pipeline/results/figures/amazing_figures/bladder/survival/TP53_CH_ctDNA_distribution.png', dpi=300, bbox_inches='tight')
