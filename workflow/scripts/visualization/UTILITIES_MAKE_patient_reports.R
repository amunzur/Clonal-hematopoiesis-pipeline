...
MAKE_pies <- function(df_main, color_what, colors_vectors, pie_title) {

	df_table <- table(df_main %>% select(all_of(color_what)))
	df <- data.frame(type = names(df_table), values = unname(df_table[1]))
	names(df) <- c(color_what, "Values")

	df <- df %>% group_by(paste(color_what)) %>%
					mutate(percent_values = (Values / sum(Values))*100)

	p <- ggplot(data = df, aes_string(x = color_what, y = "percent_values", fill = color_what)) + 
		geom_bar(stat = "identity") + 
		scale_fill_manual(values = colors_vectors) +
		blank_theme + 
		coord_polar("y", start = 0) + 
		ggtitle(pie_title)

	# if there are more than one type, add the percentages
	if (nrow(df) > 1) {
		p <- p + geom_text(aes(y = Values/3 + c(0, cumsum(Values)[-length(Values)]), 
