...
clin = clin[~pd.isnull(clin[col])].reset_index(drop = True)
...
chip_status = annotate_mutation_status(ch[ch["VAF_n"] >= 10].reset_index(drop = True), "Kidney", PATH_sample_information, annotate_what = "CHIP", annotate_gene = False, drop_dependent = True)
...
data = clin.merge(ctdna_status, how = "left").\
    merge(ch_status, how = "left").\
...
run_fishers_exact(data, "ctDNA status", col)
